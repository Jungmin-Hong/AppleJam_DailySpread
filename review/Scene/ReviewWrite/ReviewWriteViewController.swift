...
final class ReviewWriteViewController: UIViewController {
    ...
    private lazy var newsTitleButton: UIButton = {
        let button = UIButton()
        button.setTitle("뉴스 제목", for: .normal)
        button.setTitleColor(.tertiaryLabel, for: .normal)
        button.contentHorizontalAlignment = .left
        button.titleLabel?.font = .systemFont(ofSize: 23.0, weight: .bold)
        button.addTarget(self, action: #selector(didTapNewsTitleButton), for: .touchUpInside)
...
extension ReviewWriteViewController: ReviewWriteProtocol {
    ...
    func setupViews() {
        ...
        [newsTitleButton, contentsTextView, imageView].forEach { view.addSubview($0) }
        
        newsTitleButton.snp.makeConstraints{
        ...
        contentsTextView.snp.makeConstraints {
            ...
            $0.top.equalTo(newsTitleButton.snp.bottom).offset(16.0)
    ...
    func presentToSearchNewsViewController() {
        let vc = UINavigationController(rootViewController: SearchNewsViewController(searchNewsDelegate: presenter))
        present(vc, animated: true)
    }
    
    func updateViews(title: String, imageURL: URL?) {
        newsTitleButton.setTitle(title, for: .normal)
        newsTitleButton.setTitleColor(.label, for: .normal)
...
private extension ReviewWriteViewController {
    ...
    @objc func didTapNewsTitleButton() {
        presenter.didTapNewsTitleButton()
    }
}
